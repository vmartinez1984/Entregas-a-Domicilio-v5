...
{
    public class ClienteRdN : BaseRdN
    {
        ...
        public async Task<ClienteDto> ObtenerClientePorCorreo(string correo)
        {
            Persona persona;
            ClienteDto cliente;

            persona = await _repositorio.Persona.ObtenerPorCorreoAsync(correo);
            cliente = _mapper.Map<ClienteDto>(persona);
            cliente.Direccion = _mapper.Map<DireccionDto>(persona.Direcciones[0]);
